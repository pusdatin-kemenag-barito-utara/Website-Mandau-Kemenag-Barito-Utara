...
function formatDateSafe(val?: string | number | Date): string {
  if (!val) return "-";
...
export function DashboardInteractiveContent({
  ...
  }, [suratMasukTotal, suratKeluarTotal, suratMasukBulanIni, suratKeluarBulanIni, recentSuratMasuk, recentSuratKeluar]);
    ...
    async function loadFreshStats() {
      try {
        ...
        if (res.success && res.data) {
          setData({
            suratMasukTotal: res.data.totalSuratMasuk || 0,
            suratKeluarTotal: res.data.totalSuratKeluar || 0,
            suratMasukBulanIni: res.data.suratMasukBulanIni || 0,
            suratKeluarBulanIni: res.data.suratKeluarBulanIni || 0,
            recentSuratMasuk: (res.data.recentSuratMasuk as unknown as SuratMasukItem[]) || [],
            recentSuratKeluar: (res.data.recentSuratKeluar as unknown as SuratKeluarItem[]) || [],
          });
        }
      } catch (e) {
        console.error("Failed to load fresh dashboard stats:", e);
      }
    }
    loadFreshStats();
    return () => {
    ...
    <FramerProvider>
        ...
        >
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-500/20">
              ...
              <span>+{data.suratMasukBulanIni} bulan ini</span>
            </span>
          </div>
        ...
        >
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-violet-50 dark:bg-violet-500/10 text-violet-700 dark:text-violet-300 border border-violet-200/60 dark:border-violet-500/20">
              ...
              <span>+{data.suratKeluarBulanIni} bulan ini</span>
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          ...
          <div className="divide-y divide-slate-100 dark:divide-white/5 flex-1">
            {data.recentSuratMasuk.length > 0 ? (
              data.recentSuratMasuk.map((surat) => (
                <m.div
                  key={surat.id}
                  whileHover={{ x: 3 }}
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-2.5">
                    <div className="flex items-center gap-1.5 text-[11px] font-medium text-slate-500 dark:text-slate-400">
                      ...
                      <span className="truncate max-w-[200px]">{surat.asalSurat || surat.asal_surat}</span>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-semibold text-slate-400 bg-slate-100 dark:bg-white/5 px-2 py-0.5 rounded-lg">
                      ...
                      <span>{formatDateSafe(surat.createdAt || surat.created_at)}</span>
                    </div>
                  </div>
                </m.div>
              ))
        ...
        >
          ...
          <div className="divide-y divide-slate-100 dark:divide-white/5 flex-1">
            {data.recentSuratKeluar.length > 0 ? (
              data.recentSuratKeluar.map((surat) => (
                <m.div
                  key={surat.id}
                  whileHover={{ x: 3 }}
                  className="p-4 sm:p-5 hover:bg-slate-50/80 dark:hover:bg-white/[0.03] transition-all flex flex-col justify-between group"
                >
                  ...
                  <div className="flex items-center justify-between mt-2.5">
                    <div className="flex items-center gap-1.5 text-[11px] font-medium text-slate-500 dark:text-slate-400">
                      ...
                      <span className="truncate max-w-[200px]">{surat.tujuanSurat || surat.tujuan_surat}</span>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-semibold text-slate-400 bg-slate-100 dark:bg-white/5 px-2 py-0.5 rounded-lg">
                      <Calendar className="h-3 w-3 text-slate-400" />
                      <span>{formatDateSafe(surat.createdAt || surat.created_at)}</span>
